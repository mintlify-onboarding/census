...
![](/images/destinations/Screen%20Shot%202021-11-03%20at%201.46.43%20PM.png)

* Your new Google Cloud Storage connection will include a GCP service account for that specific Census connection. Click the copy button (  
...
![](/images/destinations/Screen%20Shot%202021-11-03%20at%202.02.14%20PM.png)

* Paste the credentials in the New Principals portion and select "Storage Object Admin."

![](/images/destinations/GCS%20Add.png)
...
![](/images/destinations/Cloud%20Successful%20Connection.png)
...
3. Under **Where do you want to sync data to?**, choose the name you assigned in Step 1 (we used GCS) as the **Connection**. Enter the **File Path** for the file where data will sync. The path can accept variables that will populate when the sync runs. See [File Path Variables](/destinations/google-cloud-storage#file-path-variables). Confirm the file path in the **Template Preview** field.  
\_\_  
💡  
_If the service account only has "Storage Object Creator" permissions, this file path needs to be unique per sync run_
